...
describe('ZynConsent API', () => {
  ...
  it('should retrieve consent events for an existing user (GET /consent/{userId})', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (request as any)(app).post('/consent').send({
      userId: 'testuser456',
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const res = await (request as any)(app).get('/consent/testuser456');
  ...
  it('should return 404 for a user with no consent events (GET /consent/{userId})', async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const res = await (request as any)(app).get('/consent/nonexistentuser');

    expect(res.statusCode).toEqual(404);
    expect(res.body.error).toContain(
      'No consent events found for user: nonexistentuser'
    );
  });
