...
app.post('/consent', (req: Request<{}, {}, ConsentEvent>, res: Response) => {
  ...
  if (!['granted', 'revoked'].includes(status)) {
    return res
      .status(400)
      .json({ error: 'Status must be "granted" or "revoked".' });
  }
  ...
  console.log(
    `[VERIFIABLE LOG]: Consent event recorded for user ${userId}:`,
    newConsentEvent
  );
...
app.get(
  ...
  (req: Request<{ userId: string }>, res: Response) => {
    ...
    if (!userEvents || userEvents.length === 0) {
      return res
        .status(404)
        .json({ error: `No consent events found for user: ${userId}` });
    }

    res.status(200).json({
      message: `Consent events for user ${userId}.`,
      consentEvents: userEvents.sort(
        (a, b) =>
...
const swaggerOptions = {
  swaggerDefinition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description:
        'API for managing user consent in the Minkalla ZynConsent module (MVP).',
    },
    servers: [
      {
      ...
      schemas: {
        ConsentEvent: {
          ...
          properties: {
            ...
            consentType: {
              type: 'string',
              enum: ['marketing', 'analytics', 'data_sharing'],
              description:
                'Type of consent (e.g., marketing, analytics, data_sharing).',
    ...
    paths: {
      '/consent': {
        post: {
          ...
          description:
            "This endpoint allows recording a user's consent status for a specific type. For MVP, consent data is stored in-memory.",
          requestBody: {
            required: true,
            content: {
              'application/json': {
                ...
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      message: {
                        type: 'string',
                        example: 'Consent event recorded successfully.',
                      },
                      consentEvent: {
                        $ref: '#/components/schemas/ConsentEvent',
                      },
                ...
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      error: {
                        type: 'string',
                        example: 'Invalid consent event data.',
                      },
                    },
                  },
                },
      ...
      '/consent/{userId}': {
        get: {
          ...
          description:
            'This endpoint returns a chronological list of all consent events recorded for a given user ID. For MVP, data is retrieved from in-memory storage. This endpoint helps with GDPR\'s "right to access" principle.',
          parameters: [
            {
              in: 'path',
              name: 'userId',
                ...
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      message: {
                        type: 'string',
                        example: 'Consent events for user user-abc-123.',
                      },
                      consentEvents: {
                        type: 'array',
                        items: {
